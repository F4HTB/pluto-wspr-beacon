...
## Pluto/Tezuka Cross Build

This builds `wspr-beacon` with the ARM toolchain generated by the Tezuka/F5OEO Buildroot tree.
...
Clone and build the firmware tree first:

```sh
mkdir -p "$HOME/sdr"
cd "$HOME/sdr"
git clone https://github.com/F5OEO/plutosdr-fw.git tezuka_fw
cd "$HOME/sdr/tezuka_fw/buildroot"
make
```

Clone `pluto-wspr-beacon`:

```sh
cd "$HOME/sdr"
git clone https://github.com/F4HTB/pluto-wspr-beacon.git
```

Cross-compile `wspr-beacon`:

```sh
cd "$HOME/sdr/pluto-wspr-beacon"
export BUILDROOT_DIR="$HOME/sdr/tezuka_fw/buildroot"
export CC="$BUILDROOT_DIR/output/host/bin/arm-linux-gcc"
...
Expected output should mention ARM, for example:
